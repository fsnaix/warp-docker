...
if [ ! -f /var/lib/cloudflare-warp/reg.json ]; then
  # if WARP_CONNECTOR_TOKEN is provided, register as WARP Connector
  if [ -n "$WARP_CONNECTOR_TOKEN" ]; then
    echo "Connector token found, registering as WARP Connector..."
    warp-cli connector new "$WARP_CONNECTOR_TOKEN" && echo "WARP Connector registered!"
  # otherwise use standard registration
